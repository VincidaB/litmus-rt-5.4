...
static long pfair_activate_plugin(void)
{
	...
	struct scheduling_cluster** clust;
	...
	/* NR_CPUS can be large enough such that using a stack-allocated array
	 * would cause an overflow of the 2048 byte stack frame limit. */
...
static int __init init_pfair(void)
{
	...
	for (cpu = 0; cpu < num_online_cpus(); cpu++)  {
		state = &per_cpu(pfair_state, cpu);
		hrtimer_init(&state->quantum_timer, CLOCK_MONOTONIC,
